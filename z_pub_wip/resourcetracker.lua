...
function loopRunner:OnUpdate(dt)
    ...
    if timePassed > 1000 then
       ...
       X2Chat:DispatchChatMessage(CMF_SYSTEM, "Path:" .. tostring(info[2].iconPath) .. " Grouptype: " .. tostring(info[2].groupType))
       X2Chat:DispatchChatMessage(CMF_SYSTEM, "GroupID Loop: " .. tostring(info[2].groupType))
       drawIcon(loopRunner, info[2].iconPath, info[2].groupType, 1000, 650, 
       	info2.resource2Current, -- time (bigger part of icon)
       	tostring(info2.resource1Current) .. "/" .. tostring(info2.resource1Max)) -- stacks (top blue)
       timePassed = 0
    end
end
